...
class MessageModel extends RelationModel{
	...
	public function generate($data){
		$LoginModel = D('Login');
		$data['senderinfo'] = $LoginModel ->getByTel($data['sender']);
		if($data['receiver']!=''){
			$data['receiverinfo'] = $LoginModel ->getByTel($data['receiver']);
		}
		return $data;
	}
	/**
	...
	public function getMessageCount($id){
		return $this->where("other_id =$id")->count();
	}
	/**
	 * [loading 加载评价]
	...
	public function loading($other_id,$page){
		...
		$page = ($page-1)*4;
		$List = $this->limit("$page,4")->order('time desc')->where("other_id=$other_id")->select();
	...
	public function getByReceiver($receiver){
		$Date = new \Org\Util\Date();
		$TypeModel = M('Type');
		$List = $this->relation(true)->where("receiver='$receiver' and state=1")->order('time desc')->select();
		for ($i=0; $i < count($List); $i++) { 
			$List[$i]['time'] = $Date ->timeDiff($List[$i]['time']);
			$List[$i] = $this->generate($List[$i]);
	...
	public function read($id){
		$data['state'] = '0';
		$result = $this->where("id='$id'")->data($data)->save();
		if($result!=0) return true;
		else return false; 
	}
}