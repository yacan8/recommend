    ...
    public function uv(){
        if(!isCrawler()){//如果不为网页爬虫
                ...
                if(is_numeric($id)){
                    ...
                    $cookieData = array(
                        'news_id' => $id,
                        'type' => M('News')->where(array('id'=>$id))->getField('type'),
                        'date' => $time
                    );
                    $recommendController = A('Recommend');
                    $isBrowse = $recommendController->setRecommendCookie($cookieData,'browseInfo');
                    if( !$isBrowse ) {
                        $data['news_id'] = $id;//文章Id$data['ip'] = $ip_value;//ip
                        $data['date'] = $time;//时间
                        $data['area'] = $area['country'];//地区
                        $data['ip'] = $ip_value;//ip
                        $data['user_id'] = session('?login') ? session('login') : 0;
...
//
//                        $data['news_id'] = I('post.id');//文章Id$data['ip'] = $ip_value;//ip
